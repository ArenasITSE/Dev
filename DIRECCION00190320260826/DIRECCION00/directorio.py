...
st.set_page_config(page_title="Vertice 1.0", page_icon=":material/network_node:", layout="wide")
...
def _enviar_email_credenciales(destinatario, usuario, contrasena_temp, nombre_persona=None):
    """Envía credenciales por correo (usuario + contraseña temporal)."""
    cfg = _get_config_smtp()
    if not cfg or not cfg.activo or not cfg.smtp_usuario or not cfg.smtp_clave:
        return False, "SMTP no configurado"
    try:
        ...
        msg["From"] = cfg.smtp_usuario
        msg["To"] = destinatario
        msg["Subject"] = "Acceso a Directorio ITS (Credenciales)"

        nombre_txt = (nombre_persona or "").strip()
        saludo = f"Hola {nombre_txt}," if nombre_txt else "Hola,"
        cuerpo = f"""{saludo}

Tus credenciales de acceso para Directorio ITS son:
Usuario: {usuario}
Contraseña temporal: {contrasena_temp}

Para mayor seguridad, si necesitas cambiarla usa la opción '¿Olvidaste tu contraseña?' dentro de la app web.
"""
        msg.attach(MIMEText(cuerpo, "plain"))
...
def _enviar_email_codigo(destinatario, usuario, codigo, nombre_persona=None):
    """Envía código de restablecimiento por correo incluyendo el usuario (login)."""
    cfg = _get_config_smtp()
    if not cfg or not cfg.activo or not cfg.smtp_usuario or not cfg.smtp_clave:
        return False, "SMTP no configurado"
    try:
        ...
        nombre_txt = (nombre_persona or "").strip()
        saludo = f"Hola {nombre_txt}," if nombre_txt else "Hola,"
        cuerpo = f"""{saludo} has solicitado restablecer tu contraseña.
...
# --- 2. GATE DE LOGIN ---
def _modo_publico_activo():
    """
    Modo público para publicar solo el directorio.
    Activación recomendada: URL con `?public=1` (o `?public=true`).
    Alternativa: variable de entorno `DIRECTORIO_PUBLICO=1`.
    """
# --- 2. GATE DE LOGIN ---
def _modo_publico_activo():
    """
    Modo público para publicar solo el directorio.
    Activación recomendada: URL con `?Directorio_Itse=1`
    Alternativa: variable de entorno `DIRECTORIO_PUBLICO=1`.
    """
    env_publico = os.environ.get("DIRECTORIO_PUBLICO", "").strip().lower()
    if env_publico in ("1", "true", "yes", "si"):
        return True
    try:
        qp = getattr(st, "query_params", None)
        if qp:
            v = (qp.get("Directorio_Itse") or "").strip().lower()
            if v in ("1", "true", "yes", "si"):
                return True
    except Exception:
        pass
    return False
...
modo_publico = _modo_publico_activo()
if modo_publico:
    # Saltar el login y forzar rol/estado mínimo para renderizar solo directorio.
    st.session_state.autenticado = True
    st.session_state.rol = "Publico"
    st.session_state.usuario_nombre = "Publico"
    st.session_state.personal_id = None
    st.session_state.usuario_id = None
...
persona_editar_id = None if modo_publico else st.session_state.get("persona_editar_id")
...
if modo_publico:
    # En modo público, solo mostramos el directorio (sin navegación de todo el sistema).
    tabs_lista = [":material/account_tree: Organigrama y Directorio"] if usa_emojis_color else ["Organigrama y Directorio"]
    selected_tab = tabs_lista[0]
else:
    with st.sidebar:
...
if (":material/analytics: Estadísticas" in tab_dict) or ("Estadísticas" in tab_dict):
    with tab_dict.get(":material/analytics: Estadísticas", tab_dict.get("Estadísticas")):
        ...
        # Asegurar columnas consistentes (evita KeyError si no hay registros)
        ...
        m8.metric(
            "Edificios",
            len(set(df_personal["edificio"].dropna().replace("", "Sin asignar"))),
            help="Personal distribuido por edificio",
        )
...
def render_usuarios():
    ...
    st.markdown("---")
    st.subheader(":material/mail: Crear usuarios docentes en lote")

    puestos_docentes = [p for p in session.query(Puesto).all() if _es_docente(p.nombre)]
    puestos_docentes_multip = [p for p in puestos_docentes if getattr(p, "multipuesto", False)]
    puestos_docentes_opts = puestos_docentes_multip if puestos_docentes_multip else puestos_docentes

    if not puestos_docentes_opts:
        st.info("No hay puestos configurados como DOCENTE/INVESTIGADOR.")
    else:
        with st.form("form_docentes_lote", clear_on_submit=True):
            st.caption("El sistema crea usuarios (rol `Empleado`) para el personal que esté en el puesto seleccionado y envía el correo.")
            puesto_lote_id = st.selectbox(
                "Plantilla docente (Puesto)*",
                options=[p.id for p in puestos_docentes_opts],
                format_func=lambda pid: next((f"{pp.nombre} ({pp.unidad.nombre})" for pp in puestos_docentes_opts if pp.id == pid), str(pid)),
                key="doc_lote_puesto"
            )
            filtro_jalon = st.text_input(
                "Filtro opcional (jalón / programa): busca dentro de `Programas Educativos` (contiene)",
                placeholder="Ej. 'Enero 2026' o el texto que uses en programas...",
                key="doc_lote_filtro"
            )
            usar_correo = st.selectbox(
                "Correo a usar",
                ["Institucional", "Personal", "Institucional o Personal"],
                index=2,
                key="doc_lote_correo"
            )
            usuario_login = st.selectbox(
                "Usuario (login) - cómo generarlo",
                ["Auto (local del correo disponible)", "Local de correo institucional", "Local de correo personal", "numero_empleado"],
                index=0,
                key="doc_lote_login"
            )
            envio_pass = st.selectbox(
                "Envío de acceso",
                ["Código para restablecer (recomendado)", "Contraseña temporal en el correo"],
                index=0,
                key="doc_lote_envio"
            )
            modo_existente = st.selectbox(
                "Si ya existe usuario para este Personal",
                ["Omitir", "Reenviar acceso (código o contraseña)"],
                index=0,
                key="doc_lote_existente"
            )

            submit_lote = st.form_submit_button("Generar usuarios y enviar correos")
            if submit_lote:
                cfg = _get_config_smtp()
                if not cfg or not cfg.activo:
                    st.error("Activa SMTP en Configuración antes de enviar correos.")
                    st.stop()

                # 1) Obtener docentes del puesto seleccionado
                docentes_q = session.query(Personal).filter(Personal.puesto_id == puesto_lote_id)
                filtro_txt = (filtro_jalon or "").strip()
                if filtro_txt:
                    docentes_q = docentes_q.filter(Personal.programas_educativos.ilike(f"%{filtro_txt}%"))
                docentes_lote = docentes_q.all()

                if not docentes_lote:
                    st.warning("No se encontraron docentes/personal para este filtro.")
                    st.stop()
                ...
                docentes_ids = [p.id for p in docentes_lote]
                usuarios_por_persona = {
                    u.personal_id: u
                    for u in session.query(UsuarioSistema).filter(UsuarioSistema.personal_id.in_(docentes_ids)).all()
                    if u.personal_id
                }
                usuarios_existentes = {u.usuario.lower() for u in session.query(UsuarioSistema).all() if u.usuario}
                usuarios_en_batch = set()

                def _nombre_personal(p):
                    return f"{p.nombre or ''} {p.apellido_paterno or ''}".strip() or None

                def _split_local(email_norm):
                    return (email_norm.split("@", 1)[0] if email_norm and "@" in email_norm else "").strip()

                for p in docentes_lote:
                    # 2) Seleccionar correo
                    email_inst = (p.correo_institucional or "").strip()
                    email_per = (p.correo_personal or "").strip()
                    if usar_correo == "Institucional":
                        email_raw = email_inst
                    elif usar_correo == "Personal":
                        email_raw = email_per
                    else:
                        email_raw = email_inst or email_per

                    val_email = _validar_email(email_raw)
                    if not val_email["ok"]:
                        omitidos += 1
                        continue
                    email_norm = val_email["datos"]["email_norm"]
                    correo_dest = email_norm

                    nombre_p = _nombre_personal(p)

                    # 3) Calcular login
                    usuario_base = None
                    if usuario_login == "numero_empleado":
                        usuario_base = (p.numero_empleado or "").strip()
                    elif usuario_login == "Local de correo institucional":
                        usuario_base = _split_local(email_inst.lower()) if email_inst else ""
                    elif usuario_login == "Local de correo personal":
                        usuario_base = _split_local(email_per.lower()) if email_per else ""
                    else:
                        # local del correo disponible
                        usuario_base = _split_local(email_norm)

                    usuario_base = (usuario_base or "").strip()
                    usuario_base = re.sub(r"\s+", "", usuario_base)
                    if not usuario_base:
                        omitidos += 1
                        continue

                    usuario_lower = usuario_base.lower()
                    if usuario_lower in usuarios_en_batch or usuario_lower in usuarios_existentes:
                        # Evitar colisiones: sufijo con id
                        usuario_lower = f"{usuario_lower}_{p.id}"
                    usuario_final = usuario_lower
                    usuarios_en_batch.add(usuario_final)

                    u_exist = usuarios_por_persona.get(p.id)
                    if u_exist:
                        if modo_existente != "Reenviar acceso (código o contraseña)":
                            omitidos += 1
                            continue

                        # Reenviar acceso a usuario existente
                        if envio_pass == "Código para restablecer (recomendado)":
                            session.query(TokenRestablecimiento).filter(TokenRestablecimiento.usuario_id == u_exist.id).delete()
                            cod = _generar_token_restablecimiento()
                            session.add(TokenRestablecimiento(usuario_id=u_exist.id, token=cod, expira_en=datetime.now() + timedelta(minutes=15)))
                            session.commit()
                            ok, err = _enviar_email_codigo(correo_dest, u_exist.usuario, cod, nombre_persona=nombre_p)
                            if ok:
                                nombres_ok += 1
                            else:
                                errores += 1
                            ...
                            u_exist.password = _hash_password(temp_pw)
                            session.commit()
                            ok, err = _enviar_email_credenciales(correo_dest, u_exist.usuario, temp_pw, nombre_persona=nombre_p)
                            if ok:
                                nombres_ok += 1
                            else:
                                errores += 1
                    ...
                    # Crear usuario nuevo
                    temp_pw = secrets.token_urlsafe(9)
                    u_nuevo = UsuarioSistema(usuario=usuario_final, password=_hash_password(temp_pw), rol="Empleado", personal_id=p.id)
                    session.add(u_nuevo)
                    session.commit()
                    creados += 1
                    ...
                    if envio_pass == "Código para restablecer (recomendado)":
                        cod = _generar_token_restablecimiento()
                        session.add(TokenRestablecimiento(usuario_id=u_nuevo.id, token=cod, expira_en=datetime.now() + timedelta(minutes=15)))
                        session.commit()
                        ok, err = _enviar_email_codigo(correo_dest, u_nuevo.usuario, cod, nombre_persona=nombre_p)
                    else:
                        ok, err = _enviar_email_credenciales(correo_dest, u_nuevo.usuario, temp_pw, nombre_persona=nombre_p)
                ...
                st.success(
                    f"Proceso terminado. Enviados: {nombres_ok}. Creados: {creados}. Omitidos: {omitidos}. "
                    f"Errores: {errores}."
                )
